...
const CALENDAR_PRODUCT_ID = "-//OR Music Events//Admin Calendar Feed//EN";
...
const calendarEvent = (event, options) => {
  ...
  const lastModified = event.lastModified
    ? formatUtcDateTime(event.lastModified)
    : options.generatedAt;
  ...
  return [
    ...
    rawProperty("LAST-MODIFIED", lastModified),
    rawProperty("SEQUENCE", Math.max(0, Number(event.sequence || 0))),
    ...dateLines,
    property("SUMMARY", event.summary || "OR Music Event"),
    event.location ? property("LOCATION", event.location) : null,
    event.description ? property("DESCRIPTION", event.description) : null,
    event.url ? property("URL", event.url) : null,
    event.calendarStatus ? rawProperty("STATUS", event.calendarStatus) : null,
    event.categories?.length
      ? property("CATEGORIES", event.categories.join(", "))
      : null,
    "END:VEVENT",
  ].filter(Boolean);
};

const buildIcsCalendar = (events = [], options = {}) => {
