...
const Cart = ({ items, setItems }) => {
  ...
  return (
    <Container>
        ...
        items.map((item) => (
          <Card key={item.id} style={{ marginBottom: '1rem' }}>
            <Card.Body>
              ...
              <Button variant="secondary" onClick={() => handleQuantityToggle(item, true)}>
                +
              </Button>{' '}
              <Button variant="secondary" onClick={() => handleQuantityToggle(item, false)}>
                -
              </Button>{' '}
      ...
      <Button variant="primary" onClick={handleInquiry} disabled={items.length === 0}>
        Send Inquiry
      </Button>
      { <PreviousInquiries />}
    </Container>
